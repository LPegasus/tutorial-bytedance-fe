...
import ora from 'ora';
import { JsonFile, FileSystem, StringBuilder, } from '@rushstack/node-core-library';
...
export class SearchUserCommandLineAction extends CommandLineAction {
    ...
    onDefineParameters() {
        ...
        this._outJsonFile = this.defineStringParameter({
            ...
            description: '指定输出文件路径，如果未指定，默认为 <当前路径>/github-user.log',
        });
    }
    async onExecute() {
        ...
        for (const name of loginNameList) {
            ...
            const frontEndRepos = datum.repositories.nodes.filter((d) => d.primaryLanguage?.name &&
            ...
            if (outputAsJson) {
                ...
                sb.append(`创建日期：${dayjs(outputData.createdAt).format('YYYY/MM/DD')}\n`);
...
function isFrontEndProject(language) {
